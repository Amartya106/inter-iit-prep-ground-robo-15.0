...
def rollout(model, venv, base_env, episodes, phase_cfg, start_index):
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            # honest per-episode grasp tracking (see grasp_failure_rate's
            # docstring below for why that metric alone is misleading):
            # s["grasped"] is the ground-truth constraint state, sampled every
            # step, not derived from the attempt/failure counters.
            grasped_now = bool(s["grasped"])
        ...
        rows.append(dict(
            ...
            ever_grasped=int(ever_grasped),
            n_grasp_events=n_grasp_events,
            grasp_retention=(grasped_steps / steps_since_first_grasp) if steps_since_first_grasp else float("nan"),
        ))
    return rows
...
def summarize(rows):
    ...
    return dict(
        ...
        # NOTE: grasp_failures/grasp_attempts, where "attempt" fires on ~every
        # ungrasped step the policy wants to grip (env.py's `want` gate is true
        # across 60% of the action range) -- so this is dominated by episode
        # length, not grasp skill. Proof: the SOLVED phase 2 (0.995 success)
        # reads 0.847 here. ever_grasped/n_grasp_events/grasp_retention below
        # are the trustworthy per-episode grasp metrics; prefer those.
        ...
        ever_grasped_rate=float(arr["ever_grasped"].mean()),
        n_grasp_events_mean=float(arr["n_grasp_events"].mean()),
        grasp_retention_mean=float(np.nanmean(arr["grasp_retention"])),
    )
...
def main():
    ...
    for cond in conditions:
        ...
        print(f"[{cond:>14}] success={summ['success_rate']:.3f} "
              ...
              f"ever_grasped={summ['ever_grasped_rate']:.3f} "
              f"grasp_events={summ['n_grasp_events_mean']:.2f} "
              f"grasp_retention={summ['grasp_retention_mean']:.3f}")
        venv.close()
    ...
    fields = ["phase", "condition", "n", "success_rate", "collision_rate", "mean_steps",
              ...
              "ever_grasped_rate", "n_grasp_events_mean", "grasp_retention_mean"]
